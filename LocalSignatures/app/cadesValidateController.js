app.controller('cadesValidateController', ['$scope', '$http', '$modal', 'blockUI', 'lib', function ($scope, $http, $modal, blockUI, lib) {
    ...
    $scope.open = function () {
        ...
        pki.openCades({
            ...
            dateReference: null,//'2017/01/19',
            acceptablePolicies: pki.cadesAcceptablePolicies.pkiBrazil,
            originalFileId: $scope.originalFile.id,
            trustArbitrators: [
                // ICP-Brasil
    ...
    $scope.selectOriginalFile = function () {
        pki.showFileBrowser({
            dialogTitle: 'Selecione o arquivo original assinado',
        }).success(function (result) {
            if (!result.userCancelled) {
                $scope.originalFile = result.files[0];
            }
        });
    };
