app.controller('cadesSignerController', ['$scope', '$http', '$modal', 'blockUI', 'lib', function ($scope, $http, $modal, blockUI, lib) {
    ...
    var fileIndex = -1;
    $scope.state = 'input';
    $scope.includeEncapsulatedContent = true;
    $scope.autoDetectCosign = true;
	...
	$scope.sign = function () {
	 ...
	 var signNextFile = function () {
        ...
        if (fileIndex < $scope.files.length) {
            ...
            pki.signCades({
                fileId: file.id,
                autoDetectCosign: $scope.autoDetectCosign,
                includeEncapsulatedContent: $scope.includeEncapsulatedContent,
                output: {
                    mode: 'autoSave',
